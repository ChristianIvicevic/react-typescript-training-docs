...
For even more advanced types please refer to the chapter <<advanced-typescript.adoc#,Advanced Typescript>>.
...
+
----
error: Property 'speak' does not exist on type 'Human | Insect'.
...
[NOTE]
====
To actually distinguish between union types you have to use so-called <<advanced-typescript.adoc#type-guards,type guards>>.
====
...
class Button extends Component<ButtonProps, ButtonState> { // <1>
    ...
    // Our custom event handler for the onClick event which is wired up in the render method above.
    private readonly onClick = () => {
        // We have access to the setState method of the parent class and can set the state variable.
        this.setState({ isActive: true, doesNotExist: 13 });
        //                              ~~~~~~~~~~~~
        // error: Property 'doesNotExist' does not exist on type 'ButtonState'.
    };
