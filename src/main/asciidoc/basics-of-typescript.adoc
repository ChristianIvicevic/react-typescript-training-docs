...
const /* @CompilerError(error: Type '"wrong type"' is not assignable to type 'boolean'.) */variable3/**/: boolean = "wrong type";
...
const result2: boolean = isThirteen(/* @CompilerError(error: Argument of type '"42"' is not assignable to parameter of type 'number'.) */"42"/**/);
const /* @CompilerError(error: Type 'boolean' is not assignable to type 'string'.) */result3/**/: string = isThirteen(42);
----
...
However the snippets will display small type hints for your reading experience indicating which types are infered.
The previous snippet with these type hints looks as follows:

[source,typescript]
----
const variable1/* @TypeHint(number) */ = 42;
const variable2/* @TypeHint(string) */ = "Hello, Typescript!";

/**
 * This function takes a number as an argument and returns a boolean.
 */
function isThirteen(arg: number)/* @TypeHint(boolean) */ {
    return arg === 13;
}

const result1/* @TypeHint(boolean) */ = isThirteen(42);
----
====
...
[source,typescript]
----
const list/* @TypeHint(number[]) */ = [3, 1, 4, 1, 5];
----
...
list./* @CompilerError(error: Property 'push' does not exist on type 'readonly number[]'.) */push/**/(9);
/* @CompilerError(error: Index signature in type 'readonly number[]' only permits reading.) */list[2]/**/ = 0;
----
...
const newPi/* @TypeHint(number[]) */ = [...pi];
----
====
...
const entitiesCopy/* @TypeHint(Entity[]) */ = [...entities]; // <2>
entitiesCopy[1].id = 13; // <3>
const isSame/* @TypeHint(boolean) */ = entities[1].id === entitiesCopy[1].id; // <4>
----
...
const entitiesCopy/* @TypeHint(Entity[]) */ = [...entities]; // <2>
entitiesCopy[1]./* @CompilerError(error: Cannot assign to 'id' because it is a read-only property.) */id/**/ = 13;
entitiesCopy[1] = { id: 13 }; // <3>
const isSame/* @TypeHint(boolean) */ = entities[1].id === entitiesCopy[1].id; // <4>
----
...
const kvPair3: [string, number] = ["key", /* @CompilerError(error: Type 'string' is not assignable to type 'number'.) */"value"/**/];
----
...
[source,typescript]
----
const noValue = kvPair2[/* @CompilerError(error: Tuple type '[string, number]' of length '2' has no element at index '5'.) */5/**/];
----
...
const someShape/* @TypeHint(Shape) */ = Shape.Circle;
const someOtherShape = Shape./* @CompilerError(error: Property 'DoesNotExist' does not exist on type 'typeof Shape'.) */DoesNotExist/**/;
----
...
const shapeName/* @TypeHint(string) */ = Shape[1];
...
const colors/* @TypeHint(Color[]) */ = [Color.Red, Color.Black, Color.Green, Color.White];
----
...
const value1/* @TypeHint(any) */ = variable.call.some.method.thatDoesntExist();
const value2/* @TypeHint(any) */ = variable[3];
const value3/* @TypeHint(any) */ = new variable();
----
...
const value1 = /* @CompilerError(error: Object is of type 'unknown'.) */variable/**/.call.some.method.thatDoesntExist();
const value2 = /* @CompilerError(error: Object is of type 'unknown'.) */variable/**/[3];
const value3 = new /* @CompilerError(error: Object is of type 'unknown'.) */variable/**/();

const anyVariable: any = variable;
const unknownVariable: unknown = variable;
const /* @CompilerError(error: Type 'unknown' is not assignable to type 'boolean'.) */boolVariable/**/: boolean = variable;
----
...
[source,typescript]
----
const mightBeUndefined = /* ... */; // <1>

const value1 = /* @CompilerError(error: Object is possibly 'undefined'.) */mightBeUndefined/**/.getValue();

if (mightBeUndefined !== undefined) {
    const value2 = mightBeUndefined.getValue(); // <2>
}
----
<1> Without explicitly denoting the type of this variable, assume that it might be `undefined`.
<2> This works just fine, since Typescript knows the object definitely isn't `undefined` in this scope and we even get code completion when attempting to call `getValue`.
...
[source,typescript]
----
const neverArray/* @TypeHint(never[]) */ = [1, 2, 3].reduce((acc, next) => acc, []);
//                                                                  ^^ <1>
----
...
[source,typescript]
----
const array = [1, 2, 3].reduce((acc, next) => /* @CompilerError(error: Type 'number' is not assignable to type 'never'.) */[...acc, next]/**/, []);
----
...
[source,typescript]
----
const array/* @TypeHint(number[]) */ = [1, 2, 3].reduce<number[]>((acc, next) => [...acc, next], []);
//                                          ^^^^^^^^ <1>
----
...
const entity/* @TypeHint(Human | Insect) */ = /* ... */;
entity.move();
entity./* @CompilerError(error: Property 'speak' does not exist on type 'Human | Insect'.\nProperty 'speak' does not exist on type 'Insect'.) */speak/**/();
----
...
[source,typescript]
----
const numberToString/* @TypeHint(string) */ = (arg: number) => `${arg}`;
const booleanToString/* @TypeHint(string) */ = (arg: boolean) => `${arg}`;
const dateToString/* @TypeHint(string) */ = (arg: Date) => `${arg}`;
----
...
[source,typescript]
----
const toString/* @TypeHint(string) */ = (arg: number | boolean | Date) => `${arg}`;
----
...
[source,typescript]
----
const toString/* @TypeHint(string) */ = <ArgType>(arg: ArgType) => `${arg}`; // <1>
----
...
[source,typescript]
----
const string1/* @TypeHint(string) */ = toString<number>(42);
const string2/* @TypeHint(string) */ = toString<boolean>(false);
const string3/* @TypeHint(string) */ = toString<number>(/* @CompilerError(error: Argument of type '"string"' is not assignable to parameter of type 'number'.) */"string"/**/);
const string4/* @TypeHint(string) */ = toString(42);
//                                  ^^ <1>
----
...
const isThirteen: Predicate<number> = (arg/* @TypeHint(number) */) => arg === 13; // <2>
//                                     ^^^ <3>

const array/* @TypeHint(number[]) */ = [1, 2, 13, 4, 13, 5];
const thirteens/* @TypeHint(number[]) */ = array.filter(isThirteen);
const containsThirteen/* @TypeHint(boolean) */ = array.some(isThirteen);
----
...
abstract class Component<Props, State> { // <1>
    ...
    public setState(newState: State)/* @TypeHint(void) */ { // <4> <5>
        /* ... */
    }
}
----
...
class Button extends Component<ButtonProps, ButtonState> { // <1>
    public void render()/* @TypeHint(JSX.Element) */ { // <2>
        return (
            <button // <3>
    ...
    private readonly onClick = ()/* @TypeHint(void) */ => { // <7>
        this.setState({ isActive: true, /* @CompilerError(error: Property 'doesNotExist' does not exist on type 'ButtonState'.) */doesNotExist/**/: 13 }); // <8>
    };
