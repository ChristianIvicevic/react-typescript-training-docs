...
type UnfoldExample1 = Unfold<number[]>;
// Equivalent to string
type UnfoldExample2 = Unfold<string[]>;
// Equivalent to boolean, since the input is not an array
type UnfoldExample3 = Unfold<boolean>;
----
...
The type `Unfold<T>` checks whether the supplied type argument `T` is any array using the `T extends (infer U)[]` expression.
...
In the second conditional expression `Unfold<T>` checks whether the supplied argument `T` is any read-only array using the `T extends readonly (infer U)[]` expression and returns either `U` or `T` accordingly.
This will now work correctly for expressions such as `Unfold<readonly boolean[]>` that haven't been tracked with the initial version.
