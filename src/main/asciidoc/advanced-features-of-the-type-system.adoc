...
Using the <<advanced-features-of-the-type-system.adoc#index-type-query-operator,index type query operator>> `keyof Factory` we can iterate over all properties `P in keyof Factory` and append the question mark to each property.
...
There exists an inverse type for `Partial` that makes all properties required, by removing the question marks using the `-?` notation as follows:
