...
Typescript provides syntactical elements to simplify common type transformations described the the upcoming example.
...
First we need a new type where all properties are optionalized:
...
Based on this type we can declare the `updateFactory` function as follows:

[source,typescript]
----
const updateFactory = (id: number, newFactoryVariables: FactoryPartial) => { /* ... */ }

// Move factory with ID = 1 to Munich
updateFactory(1, { location: "Munich" });
----

Manually writing such a `FactoryPartial` type, especially for other types used throughout the application is a mundane task.
The syntax to automate this in order to map `Factory` to an optionalized version is the following:
...
At its core the mapped type syntax describes how to iterate over all properties and which transformations to perform.
Using the <<advanced-features-of-the-type-system.adoc#index-type-query-operator,index type query operator>> `keyof Factory` we can iterate over all properties `P in keyof Factory` and append the question mark.
The type of the prop is denoted using the <<advanced-features-of-the-type-system.adoc#indexed-access-operator,indexed access operator>> `Factory[P]`.
Since this transformation is commonly used it is possible to extract it into a generic type as follows:
...
The `Partial` type has an inversion to make all properties required, by removing the question marks using the `-?` notation as follows:
...
Another very common mapped type is `Readonly`:
...
Imagine the task to write a type mapping `Unfold<T>` that maps any array type `T = U[]` to `U` itself, otherwise just `T`.
In this case we can leverage the ternary syntax `A extends B ? C : D` used as follows:
...
Just writing `U[]` is wrong since Typescript thinks that `U` refers to a previously declared type.
...
Using this snippet an expression such as `Unfold<number[]>` yields `number` while `Unfold<string>` yields `string`.
...
The new snippet will now work correctly for mutable arrays and read-only arrays in expressions such as `Unfold<readonly boolean[]>`.
