...
[source,typescript]
----
const entity/* @TypeHint(Client | Project) */ = /* ... */;

const entityId/* @TypeHint(number) */ = entity.id;
const entityName/* @TypeHint(string) */ = entity.name;
const entityDescription = entity./* @CompilerError(error: Property 'description' does not exist on type 'Client | Project'.\nProperty 'description' does not exist on type 'Client'.) */description/**/;
const entityPhoneNumber = entity./* @CompilerError(error: Property 'phoneNumber' does not exist on type 'Client | Project'.\nProperty 'phoneNumber' does not exist on type 'Project'.) */phoneNumber/**/;
----
...
[source,typescript]
----
const entity/* @TypeHint(Client | Project) */ = /* ... */; // <1>

const entityId/* @TypeHint(number) */ = entity.id; // <2>
const entityName/* @TypeHint(string) */ = entity.name; // <2>

if (isProject(entity)) { // <3>
    const entityDescription/* @TypeHint(string) */ = entity.description; // <4>
} else {
    const entityPhoneNumber/* @TypeHint(string) */ = entity.phoneNumber; // <5>
}
----
...
[source,typescript]
----
const response/* @TypeHint(SearchResponse) */ = /* ... */;

response.forEach((searchEntry/* @TypeHint(SearchEntry) */) => {
    if (/* @CompilerError(error: This condition will always return 'false' since the types '"Client" | "Project" | "Factory"' and '"Porject"' have no overlap.) */searchEntry.kind === "Porject"/**/) {
        // ...
    }
    // ...
});
...
const rainbowFactory/* @TypeHint(Factory) */ = /* ... */;
...
const errorTuple = pluck(rainbowFactory, ["name", /* @CompilerError(error: Type 'string' is not assignable to type '"id" | "name" | "description" | "location"'.) */"something"/**/]);
----
